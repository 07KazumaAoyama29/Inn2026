...
これは、`word`に`"こんにちは"`という文字を入れるという意味です。<br>
...
この場合、条件が`False`になるので、何も表示されません。<br>
これはエラーではありません。<br>
if文の中の処理は、条件式が`True`の時だけ実行されます。<br>

# elseを復習しよう
## 条件を満たさなかったときの処理<br>
`if`だけでは、条件式が`True`のときだけの処理しか書けません。<br>
`False`のときにしてほしい処理を書くには、`else`を使います。<br>
...
好きな数字を入力して、`Enterキー`を押してください。<br>

## STEP2 input文の説明
最初のプログラムは、変数に、入力された文字を代入しています。
...
→なので、同じ文字が2回`print`されているように見えます。

## STEP3 inputの中に文字を入れてみよう
`input()` の中には、入力するときに表示する**案内文**を書くことができます。<br>
